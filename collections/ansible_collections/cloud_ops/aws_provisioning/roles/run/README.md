Cloud_ops.Aws_provisioning Run Role
========================
...
- name: Execute tasks on servers
  hosts: servers
  roles:
    - role: cloud_ops.aws_provisioning.run
      run_x: 42
```
...
```yaml
- name: Initialize the run role from cloud_ops.aws_provisioning
  hosts: servers
  gather_facts: false
  tasks:
    - name: Trigger invocation of run role
      ansible.builtin.include_role:
        name: cloud_ops.aws_provisioning.run
      vars:
        run_x: 42
```
